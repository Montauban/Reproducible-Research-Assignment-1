...
```{r echo=FALSE}
cat("Mean Steps per day:", steps_mean)
```
...
```{r echo=FALSE}
cat("Median steps value:", steps_median)
```
...
```{r echo=FALSE}
cat("The 5 minute interval that contains the most steps is at", data$interval[104], "am",
  "with", max(ordered_interval$Average_Steps), "steps")
```
...
```
```{r echo=FALSE}
cat("The total number of missing values in the data set is", total_na_steps, ".")
```
